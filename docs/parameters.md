...
| `--manifest` | required | JSONL manifest produced by `irodori-prepare-manifest`. Each row must include `text` and `latent_path`; `speaker_id` and `caption` are optional depending on the model. `caption` may be either a string or a list of strings; list captions are sampled randomly each time the row is loaded. |
...
| `num_workers` / `--num-workers` | `2` | DataLoader worker count. The bundled GPU recipes use a larger host-appropriate value. |
| `dataloader_persistent_workers` | `False` | Keeps worker processes alive between epochs. The bundled GPU recipes enable it. |
| `dataloader_prefetch_factor` | `2` | Number of CPU batches prepared ahead by each worker. |
| `dataloader_prefetch_to_device` / `--prefetch-to-device` | `True` | On CUDA, overlaps the next selected pinned-memory batch transfer with current model compute. Use `--no-prefetch-to-device` for debugging or memory-constrained runs. |
...
The collator omits target latents for duration-only training, reference tensors when speaker
conditioning is disabled, and unpatched compatibility tensors in the trainer. Fast Hugging Face
tokenizers are called in batch mode automatically.
...
For inference, pass the saved adapter directory to `irodori-infer --lora-adapter`.
...
`uv run torchrun --nproc_per_node N -m irodori_tts.cli.train ...`.
...
`irodori-prepare-manifest` encodes dataset audio into DACVAE latents and writes a
JSONL manifest consumed by `irodori-train`.
