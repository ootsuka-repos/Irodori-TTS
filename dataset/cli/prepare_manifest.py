...
from dataset._textnorm import sanitize_manifest_id_component
...
def _resolve_speaker_namespace(args: argparse.Namespace) -> str:
    ...
    return sanitize_manifest_id_component(base, fallback="dataset")
...
def _prepare_example(
    ...
    try:
        ...
        # Pass provenance through so latent manifests stay traceable to source
        ...
        if args.speaker_columns:
            ...
            for speaker_column in args.speaker_columns:
                ...
                speaker_component = sanitize_manifest_id_component(speaker_raw, fallback="")
        ...
        # Move the CPU-heavy codec preprocessing (downmix, resample, loudness
        # normalization) onto prefetch workers so the encode thread only feeds
        # the GPU. The consumer then calls encode_waveform(..., normalize_db=None).
        codec_sample_rate = getattr(args, "codec_sample_rate", None)
        if codec_sample_rate is not None:
            if wav.shape[0] > 1:
                wav = wav.mean(dim=0, keepdim=True)
            if sr != int(codec_sample_rate):
                wav = torchaudio.functional.resample(wav, sr, int(codec_sample_rate))
                sr = int(codec_sample_rate)
                if wav.numel() == 0:
                    return _PreparedItem(idx=idx, status="skip", skip_reason="trimmed_empty")
            if args.normalize_db is not None:
                wav = DACVAECodec._normalize_loudness(
                    wav[0], sample_rate=sr, target_db=args.normalize_db
                ).unsqueeze(0)
...
def _iter_rank_examples(
    ...
    *,
    ...
    use_dataset_shard = shard_strategy in ("dataset", "contiguous") and hasattr(dataset, "shard")
    if use_dataset_shard and start > 0 and world_size > 1:
        # dataset.shard() enumerates shard-local indices, so a global
        # --skip-samples offset cannot be applied consistently across ranks.
        raise ValueError(
            "--skip-samples > 0 is not supported with dataset.shard() based sharding on "
            "iterable datasets; use --shard-strategy stride instead."
        )

    def _iter_stream() -> Iterable[_PreparedItem | tuple[int, dict[str, Any]]]:
        sharded = None
        if use_dataset_shard:
            # Only the shard() call itself may fall back to strided iteration.
            # Errors raised while iterating must propagate: silently restarting
            # iteration from scratch would duplicate already-processed samples.
            try:
...
def _latent_filename(idx: int, *, rank: int, world_size: int) -> str:
    """Deterministic latent file name derived from the sample index only.

    Re-runs over the same input therefore overwrite the same files instead of
    leaving orphaned latents behind, and manifest content stays reproducible
    regardless of prefetch completion order. The rank prefix is kept for
    multi-rank runs because iterable-dataset shards enumerate shard-local
    indices that may collide across ranks.
    """
    if world_size > 1:
        width = max(2, len(str(world_size - 1)))
        return f"rank{rank:0{width}d}_{int(idx):08d}.pt"
    return f"{int(idx):08d}.pt"


def _skipped_sidecar_path(manifest_path: Path) -> Path:
    return manifest_path.with_name(f"{manifest_path.name}.skipped.jsonl")


def _merge_shards(base_path: Path, world_size: int, *, keep_shards: bool) -> None:
    def _merge(shards: list[Path], destination: Path) -> None:
    ...
    manifest_shards = [_ranked_path(base_path, rank, world_size) for rank in range(world_size)]
    skipped_shards = [_skipped_sidecar_path(path) for path in manifest_shards]
    _merge(manifest_shards, base_path)
    _merge(skipped_shards, _skipped_sidecar_path(base_path))
    if not keep_shards:
        for shard_path in manifest_shards + skipped_shards:
...
def _run_worker(
    ...
    *,
    ...
    if column_names is None:
        # Iterable datasets may not expose resolved features up front; skip
        # validation instead of crashing on `in None` and let per-sample column
        # lookups handle genuinely missing columns.
        print(
            "column names are unavailable for this dataset "
            "(streaming without resolved features); skipping column validation.",
            flush=True,
        )
    else:
        if args.audio_column not in column_names:
            raise ValueError(f"audio column '{args.audio_column}' not found: {column_names}")
        if args.text_column not in column_names and not _has_column(
            ds.features, args.text_column
        ):
            raise ValueError(f"text column '{args.text_column}' not found: {column_names}")
        if (
            args.caption_column is not None
            and args.caption_column not in column_names
            and not _has_column(ds.features, args.caption_column)
        ):
            raise ValueError(f"caption column '{args.caption_column}' not found: {column_names}")
        if args.speaker_columns:
            missing_speaker_columns = [
                column
                for column in args.speaker_columns
                if column not in column_names and not _has_column(ds.features, column)
            ]
            if missing_speaker_columns:
                raise ValueError(
                    f"speaker column(s) not found: {missing_speaker_columns}; "
    ...
    # Prefetch workers downmix/resample/normalize to this rate; the encode
    # thread then passes normalize_db=None so nothing is applied twice.
    args.codec_sample_rate = int(codec.sample_rate)
    ...
    def _log_progress() -> None:
        ...
        if show_progress:
            pbar.set_postfix(
                {
                    "written": written,
                    "skip": (skipped_empty + skipped_audio + skipped_low_sr + skipped_max),
                },
                refresh=False,
            )
        else:
    ...
    def _record_skip(item: _PreparedItem, reason: str, skip_f) -> None:
        record: dict[str, Any] = {"idx": item.idx, "reason": reason}
        if item.error:
            record["error"] = item.error
        skip_f.write(json.dumps(record, ensure_ascii=False) + "\n")

    def _handle_item(item: _PreparedItem, *, stop_requested: bool, out_f, skip_f) -> None:
        nonlocal seen, written
        if item.status == "error" and item.idx < 0:
            # The dataset iterator itself failed (not a single sample); abort
            # instead of finishing with a silently incomplete manifest.
            raise RuntimeError(f"dataset iteration failed: {item.error}")
        seen += 1
        if show_progress:
            pbar.update(1)
        ...
        if item.status == "skip":
            ...
            _inc_skip(reason)
            _record_skip(item, reason, skip_f)
            _log_progress()
            return
        if item.status == "error":
            reason = item.skip_reason or "prepare_error"
            _inc_skip(reason)
            _record_skip(item, reason, skip_f)
            _log_progress()
            return
        if stop_requested:
            ...
            _record_skip(item, "max_samples_limit", skip_f)
            _log_progress()
            return
        ...
        if wav is None or sr is None or text is None:
            ...
            _record_skip(item, "prepare_error", skip_f)
            _log_progress()
            return

        try:
            with torch.inference_mode():
                # Preprocessing (downmix/resample/loudness) already happened in
                # _prepare_example; pass normalize_db=None to avoid re-applying.
                latent = codec.encode_waveform(wav, sample_rate=sr, normalize_db=None)[0].cpu()
        except Exception as exc:
            item.error = str(exc)
            _inc_skip("encode_error")
            _record_skip(item, "encode_error", skip_f)
            _log_progress()
            return

        latent_name = _latent_filename(item.idx, rank=rank, world_size=world_size)
        latent_path = (latent_dir / latent_name).resolve()
        latent_tmp = latent_path.with_name(f".{latent_path.name}.tmp")
        torch.save(latent, latent_tmp)
        os.replace(latent_tmp, latent_path)
        try:
            latent_rel = os.path.relpath(latent_path, start=manifest_base)
        except ValueError as exc:
            raise ValueError(
                "latent files and the output manifest must be on the same filesystem/drive: "
                f"{latent_path}, {output_manifest}"
            ) from exc
        payload = {
            "text": text,
            "latent_path": Path(latent_rel).as_posix(),
    ...
    skipped_manifest = _skipped_sidecar_path(output_manifest)
    tmp_manifest = output_manifest.with_name(f".{output_manifest.name}.tmp")
    tmp_skipped = skipped_manifest.with_name(f".{skipped_manifest.name}.tmp")
    completed = False
    try:
        with (
            tmp_manifest.open("w", encoding="utf-8", newline="\n") as out_f,
            tmp_skipped.open("w", encoding="utf-8", newline="\n") as skip_f,
        ):
            if args.prefetch > 0:
                ...
                while True:
                    ...
                    if args.max_samples is not None and written >= args.max_samples:
                        stop_requested = True
                        stop_event.set()
                    _handle_item(queued, stop_requested=stop_requested, out_f=out_f, skip_f=skip_f)
                thread.join()
            else:
                for entry in iter_items:
                    if args.max_samples is not None and written >= args.max_samples:
                        break
                    if isinstance(entry, _PreparedItem):
                        _handle_item(entry, stop_requested=False, out_f=out_f, skip_f=skip_f)
                        continue
                    idx, sample = entry
                    item = _prepare_example(idx, sample, args)
                    _handle_item(item, stop_requested=False, out_f=out_f, skip_f=skip_f)
        completed = True
    finally:
        pbar.close()
        if completed:
            # Publish atomically so an interrupted run never leaves a partial
            # manifest that looks complete at the final path.
            os.replace(tmp_manifest, output_manifest)
            os.replace(tmp_skipped, skipped_manifest)
        else:
            tmp_manifest.unlink(missing_ok=True)
            tmp_skipped.unlink(missing_ok=True)
    ...
    if skip_counts:
        print(f"{rank_prefix}skip breakdown (details: {skipped_manifest}):")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=True,
        help=(
            "Apply core text normalization before writing manifest text. "
