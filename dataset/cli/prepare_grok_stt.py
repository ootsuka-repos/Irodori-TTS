...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=0,
        help=(
            "Exit non-zero when dataset building records more than this many source "
            "errors (clip extraction/decode failures, missing word timestamps). "
            "Default: 0 so silently dropped rows fail the run."
        ),
    )
...
def _print_inventory(sources: list[AudioSource], *, auth_mode: str, cached: int) -> None:
...
def main() -> None:
    ...
    # Scan the raw-response cache once; the inventory print and the missing-source
    # selection below reuse it instead of re-parsing every response JSON. Passing
    # options invalidates cached transcripts made with different request settings.
    cached_source_ids = {
        source.source_id
        for source in selected
        if load_cached_response(output_dir, source, options) is not None
    }
    _print_inventory(selected, auth_mode=args.auth_mode, cached=len(cached_source_ids))
    print(
    ...
    missing = [
        source
        for source in selected
        if args.force_transcribe or source.source_id not in cached_source_ids
    ]
    ...
    if args.transcribe_only:
        ...
        # Clip extraction/decode failures silently drop rows from the manifests;
        # surface them through the exit code instead of only in source_errors.jsonl.
        source_error_count = int(
            summary.get("source_error_count", summary.get("source_errors", 0)) or 0
        )
        if source_error_count > args.max_source_errors:
            failures.append(
                f"dataset build recorded {source_error_count} source error(s) "
                f"(limit {args.max_source_errors}); see {output_dir / 'source_errors.jsonl'}"
            )
