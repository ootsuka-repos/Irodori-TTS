#!/usr/bin/env python3
"""Transcribe finalized clip events on CUDA (default backend: faster-whisper / whisper-ja-1.5B)."""
...
def _parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Transcribe finalized clip events locally on CUDA. The default backend is "
            "faster-whisper (TransWithAI/whisper-ja-1.5B-ct2); litagin/anime-whisper is "
            "available via --backend. Each result is cached by audio fingerprint and model revision."
        )
    )
    parser.add_argument(
        "--input-manifest",
    ...
    parser.add_argument(
        "--cache-dir",
        type=Path,
        default=Path("dataset/data/pipeline/nonverbal_events_new/transcribe_cache"),
    )
    parser.add_argument(
        "--backend",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Speech mode: make the ASR output the primary transcript. Any prior "
            "text is preserved as 'text_before_asr' and 'text' is replaced by "
            "the ASR transcript; rows whose transcript is unusable "
...
def _apply_text_replacement(rows: list[dict[str, Any]]) -> dict[str, int]:
    """Make ASR text primary in place; flag unusable rows."""
    swapped = 0
    kept = 0
    for row in rows:
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if args.replace_text:
        ...
        print(
            f"transcript text replacement swapped={replacement['swapped']} "
    ...
    print(
        f"transcription complete rows={summary['rows']} ok={summary['ok']} "
        f"review={summary['review']} errors={summary['errors']} output={output_path}"
    )
    if summary["errors"]:
        raise SystemExit(
            f"transcription completed with {summary['errors']} error(s); see {summary_path}"
        )
