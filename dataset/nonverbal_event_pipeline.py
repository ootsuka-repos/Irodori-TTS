...
from dataset.nonverbal_clustering import read_source_window, resample_mono
...
from dataset.nonverbal_subclustering import (
    SubclusteringConfig,
...
class PipelineConfig:
    ...
    # False when the visualization hook builds the review class folders itself
    # (the default CLI report does); prevents building a classes/ tree that the
    # hook immediately replaces and keeps summary["folders"] truthful.
    materialize_review_folders: bool = True
...
# All writes route through the shared fsync-and-retry helpers in ``_io_utils``;
# the wrappers below only bind the project JSON encoder.
...
def propagate_labels_scoped(
    ...
    *,
    ...
    for key in sorted(grouped, key=str.casefold):
        ...
        # Manual labels are always passed down so a human-corrected row can
        # never act as a classifier seed for its neighbors; the flag only
        # controls whether the manual label itself propagates.
        group_manual = {
        ...
        decisions = propagate_nonverbal_labels(
            ...
            config=settings,
            propagate_manual_seeds=propagate_manual_seeds,
        )
...
def _read_window(reader: sf.SoundFile, start: float, end: float, sample_rate: int) -> np.ndarray:
    mono = read_source_window(reader, start, end)
    return resample_mono(mono, int(reader.samplerate), sample_rate).numpy()


_valid_audio_file = valid_audio_file
_atomic_write_flac = atomic_write_flac
    ...
    *,
    ...
    if state_path.is_file():
        ...
        if isinstance(loaded, dict):
            state = loaded
    cache_valid = not force and state.get("classifier_fingerprint") == classifier_fingerprint
    if not cache_valid:
        # The fingerprint changed (or --force was given).  The old prediction
        # files must be removed *before* the new state is written; otherwise an
        # interrupted rerun would find a matching state and silently resume
        # from predictions computed under the previous fingerprint.
        final_path.unlink(missing_ok=True)
        partial_path.unlink(missing_ok=True)
    cached: dict[str, dict[str, Any]] = {}
    if cache_valid:
        # The final snapshot is read first, then the partial append log; later
        # rows win, so the newest prediction for a segment is kept.  Each row
        # additionally carries the fingerprint it was computed under as a
        # second line of defense; rows from other fingerprints are dropped.
        for candidate_path in (final_path, partial_path):
            if not candidate_path.is_file():
                continue
            for row in _read_jsonl(candidate_path):
                row_fingerprint = row.get("classifier_fingerprint")
                if row_fingerprint is not None and row_fingerprint != classifier_fingerprint:
                    continue
                segment_id = str(row.get("id", ""))
                if segment_id in expected_audio:
                    clean = {
                        ...
                        if key != "classifier_fingerprint"
                    }
                    cached[segment_id] = {**clean, "audio": expected_audio[segment_id]}
    ...
    for offset in range(0, len(missing), batch_size):
        ...
        for row in predicted:
            ...
            checkpoint_rows.append(
                {**cached[segment_id], "classifier_fingerprint": classifier_fingerprint}
            )
        # Append only the new batch instead of rewriting every prediction so
        # checkpointing stays O(n) over the whole run instead of O(n^2).
        append_jsonl(partial_path, checkpoint_rows, default=json_default)
        print(
    ...
    _atomic_write_jsonl(
        final_path,
        ({**row, "classifier_fingerprint": classifier_fingerprint} for row in predictions),
    )
    partial_path.unlink(missing_ok=True)
...
def run_nonverbal_event_pipeline(
    *,
    ...
    # The dataclass defaults of NonverbalLabelingConfig are the single source
    # of truth for production thresholds; no local copy is kept here.
    ...
    manual_seed_file_found = seed_path is not None and seed_path.is_file()
    if seed_path is not None and not manual_seed_file_found:
        print(
            f"warning: manual seed file not found, continuing without manual seeds: {seed_path}",
            flush=True,
        )
    ...
    fingerprint_configs = {
        ...
        "materialize_review_folders": settings.materialize_review_folders,
        "manual_seed": asdict(manual_settings),
        "labeling": asdict(label_settings),
        "merge": asdict(merge_settings),
        "subclustering": asdict(subcluster_settings),
    ...
    dropped_short_primitives = [
        row for row in published_primitives if str(row["id"]) not in retained_primitive_ids
    ]
    published_primitives = [
        row for row in published_primitives if str(row["id"]) in retained_primitive_ids
    ]
    # Keep discarded_by_label reconcilable with discarded_primitive_count by
    # also accounting for target primitives dropped with too-short events.
    discarded_by_label = Counter(
        {str(key): int(value) for key, value in selection_summary["discarded_by_label"].items()}
    )
    for row in dropped_short_primitives:
        discarded_by_label[f"short_event_{row.get('final_label')}"] += 1
    selection_summary = {
        **selection_summary,
        ...
        "discarded_by_label": dict(sorted(discarded_by_label.items())),
    }
    ...
    if settings.materialize_review_folders:
        folder_summary: dict[str, Any] = materialize_class_folders(event_rows, dataset_dir=stage)
    else:
        # The visualization hook owns the classes/ tree; building it here would
        # only be thrown away when the hook replaces it moments later.
        folder_summary = {"source": "visualization_hook"}
    clean_primitives = [
    ...
    summary["manual_seed_file_found"] = manual_seed_file_found if seed_path is not None else None
    if visualization_hook is not None:
        ...
        if visualization:
            ...
            if not settings.materialize_review_folders:
                # The hook is the sole owner of the classes/ tree, so its
                # materialization stats become the single "folders" record.
                materialization = visualization.pop("materialization", None)
                if isinstance(materialization, Mapping):
                    summary["folders"] = {
                        "source": "visualization_hook",
