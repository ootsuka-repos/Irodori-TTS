...
from dataset.nonverbal_subclustering import select_diverse_representatives
...
# Thin wrappers keep call sites short while routing every write through the
# shared fsync-and-retry helpers in ``_io_utils``.
_atomic_write_text = atomic_write_text
_atomic_save_npy = atomic_save_npy


def _atomic_write_json(path: Path, payload: Any) -> None:
    atomic_write_json(path, payload, default=json_default)


def _atomic_write_jsonl(path: Path, rows: Iterable[dict[str, Any]]) -> None:
    atomic_write_jsonl(path, rows, default=json_default)
...
def _source_key(source_uid: str) -> str:
    """Return an inventory-order-independent per-source key.

    The key intentionally contains no positional index: adding or removing one
    source must not rename the cached embedding shards of every other source.
    """
    return f"s{hashlib.sha1(source_uid.encode('utf-8')).hexdigest()[:16]}"
...
def read_source_window(
    ...
    *,
    ...
    """Read ``[start, end)`` seconds from an open file as mono float32.

    This is the single crop implementation shared by candidate embedding,
    clip export, and the event pipeline; it replaces five near-identical
    ...
    if audio.size == 0 or not np.isfinite(audio).all():
        label = context or f"{start:.6f}-{end:.6f}s"
        raise ValueError(f"Invalid audio window: {label}")
    return np.ascontiguousarray(np.mean(audio, axis=1, dtype=np.float32))
...
def resample_mono(
    ...
    """Resample a mono window with a cached kernel per (source, target) pair."""
    ...
    return resampler(waveform).contiguous()
...
def _read_gap_waveform(
    ...
    mono = read_source_window(reader, start, end, context=f"gap {start:.6f}-{end:.6f}s")
...
def load_vad_complements(
    ...
    *,
    ...
    excluded_gaps: list[dict[str, Any]] = []
    ...
    # Single pass over the cached responses: each JSON payload is parsed exactly
    # once, and a source is processed immediately when it is eligible, within
    # the max_sources cap, and inside this worker's shard slice.
    for raw_path in raw_paths:
        ...
        if source_duration <= min_source_seconds:
            ...
            continue
        eligible_index = eligible_count
        eligible_count += 1
        if max_sources is not None and eligible_index >= max_sources:
            continue
        if eligible_index % source_shard_count != source_shard_index:
            continue
        ...
        source_key = _source_key(source_uid)
        vad_regions = [
    ...
    selected_eligible = eligible_count if max_sources is None else min(eligible_count, max_sources)
    if selected_eligible <= 0:
        raise RuntimeError(
    ...
    excluded = [*excluded_sources, *excluded_gaps]
    summary = {
        ...
        "excluded_short_gaps": len(excluded_gaps),
...
def _read_candidate_audio(
    ...
    mono = read_source_window(reader, candidate.start, candidate.end, context=candidate.id)
    metrics = _audio_metrics(mono)
    waveform = resample_mono(mono, int(reader.samplerate), BEATS_SAMPLE_RATE)
...
def _prefetch_iter(iterable: Iterable[Any], *, depth: int = 2) -> Iterator[Any]:
    """Consume ``iterable`` on a worker thread so decoding overlaps GPU work.

    Producer exceptions are re-raised on the consumer side.  When the consumer
    stops early the producer is signaled to stop and the queue is drained.
    """
    queue: Queue[Any] = Queue(maxsize=max(1, depth))
    sentinel = object()
    ...
    worker = threading.Thread(target=_produce, name="beats-decode-prefetch", daemon=True)
    worker.start()
    try:
        while True:
            item = queue.get()
...
def _iter_embedding_work(
    ordered_sources: Sequence[tuple[int, str, list[CandidateWindow]]],
    *,
    shard_dir: Path,
    force: bool,
    model_samples: int,
    ...
    """Yield cached shards and decoded waveform batches in deterministic order.

    Message kinds: ``("cached", number, key, embeddings, rows)``,
    ``("batch", number, key, waveforms, rows)``, ``("end_source", number, key)``.
    """
    for source_number, source_key, source_candidates in ordered_sources:
        ...
        if not force and embedding_path.is_file() and metadata_path.is_file():
            ...
            if cached_embeddings.shape[0] == len(expected_ids) and cached_ids == expected_ids:
                yield ("cached", source_number, source_key, cached_embeddings, cached_rows)
                continue
        ...
        with sf.SoundFile(source_path) as reader:
            for candidate in source_candidates:
                waveform, metrics = _read_candidate_audio(reader, candidate)
                row = {
                    **asdict(candidate),
                    "outside_vad": True,
                ...
                if len(batch_waveforms) >= batch_size:
                    yield ("batch", source_number, source_key, batch_waveforms, batch_rows)
                    batch_waveforms = []
                    batch_rows = []
            if batch_waveforms:
                yield ("batch", source_number, source_key, batch_waveforms, batch_rows)
        yield ("end_source", source_number, source_key)
...
def extract_beats_embeddings(
    ...
    *,
    ...
    prune_stale_shards: bool = True,
) -> tuple[np.ndarray, list[dict[str, Any]]]:
    """Extract resumable, per-source BEATs embedding shards.

    ``prune_stale_shards`` must be false for sharded feature workers: a worker
    only sees its own slice of the inventory and would otherwise delete every
    other worker's completed shards.
    """
    if batch_size <= 0:
    ...
    ordered_sources = [
        (source_number, source_key, sorted(grouped[source_key], key=lambda c: (c.start, c.end)))
        for source_number, source_key in enumerate(sorted(grouped), 1)
    ]
    source_embeddings: list[np.ndarray] = []
    source_rows: list[dict[str, Any]] = []

    work = _iter_embedding_work(
        ordered_sources,
        shard_dir=shard_dir,
        force=force,
    ...
    for message in _prefetch_iter(work, depth=2):
        kind = message[0]
        if kind == "cached":
            _, source_number, source_key, cached_embeddings, cached_rows = message
            ...
            print(
                ...
                f"windows={len(cached_rows)} key={source_key}",
                flush=True,
            )
        elif kind == "batch":
            _, _source_number, _source_key, batch_waveforms, batch_rows = message
            if model is None:
            ...
            _, source_number, source_key = message
            ...
            _atomic_save_npy(shard_dir / f"{source_key}.npy", embedding_array)
            _atomic_write_jsonl(shard_dir / f"{source_key}.jsonl", source_rows)
...
def _select_representatives(
    ...
    *,
    ...
    """Delegate to the shared, deduplicating selector.

    The previous local implementation could select the same member twice when
    a cluster had fewer distinct sources than requested representatives; the
    shared selector never repeats an index.
    """
...
def _export_audio_window(row: dict[str, Any], output_path: Path) -> np.ndarray:
    ...
    with sf.SoundFile(source_path) as reader:
        mono = read_source_window(
            reader,
            float(row["start"]),
            float(row["end"]),
            context=str(row.get("id", source_path)),
        )
        waveform = resample_mono(mono, int(reader.samplerate), OUTPUT_SAMPLE_RATE)
    output = waveform.numpy()
    atomic_write_flac(output_path, output, OUTPUT_SAMPLE_RATE)
    return output
...
def _read_mono_window(reader: sf.SoundFile, row: dict[str, Any]) -> torch.Tensor:
    ...
    return resample_mono(mono, int(reader.samplerate), OUTPUT_SAMPLE_RATE)
    ...
    for source_number, source_key in enumerate(sorted(grouped), 1):
        ...
        with sf.SoundFile(source_path) as reader:
            for row in source_rows:
                ...
                # A crashed previous run can leave a truncated file behind;
                # only a decodable clip counts as reusable, and new clips are
                # committed atomically so members/ never links a partial file.
                if valid_audio_file(clip_path):
                    clips_reused += 1
                else:
                    waveform = _read_mono_window(reader, row)
                    atomic_write_flac(clip_path, waveform.numpy(), OUTPUT_SAMPLE_RATE)
                    clips_written += 1
...
canvas.addEventListener('click',ev=>{let i=nearest(ev);if(i<0)return;let p=points[i];details.textContent=`ID: ${p[4]}\nHDBSCAN: ${p[2]??'noise'}\nKMeans: ${p[3]}\nSource: ${p[6]}\nTime: ${p[7].toFixed(3)}–${p[8].toFixed(3)} sec\nRMS: ${p[9].toFixed(2)} dBFS`;if(p[5]){player.src='../'+p[5];player.play().catch(()=>{})}else{player.removeAttribute('src');details.textContent+='\n(音声クリップ未出力: --no-export-all-clips)'}});
...
def run_nonverbal_clustering(
    *,
    ...
    # Pruning stale shard files is only safe when this run has seen the whole
    # inventory: sharded feature workers and capped runs each see a subset and
    # must never delete the other workers' completed shards.
    prune_stale_shards = (
        not features_only and source_shard_count == 1 and max_sources is None
    )
    embeddings, rows = extract_beats_embeddings(
        ...
        prune_stale_shards=prune_stale_shards,
    )
    if features_only:
        summary = {
