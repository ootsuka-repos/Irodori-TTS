...
def _merge_short_fragments(
    ...
    *,
    ...
    while len(boundaries) > 2:
        short_pair: tuple[int, int] | None = None
        for left_index in range(len(boundaries) - 1):
            ...
            if (
                ...
                short_pair = (left_index, right_index)
                break
        if short_pair is None:
            return
        left_index, right_index = short_pair
...
def _split_long_fragments(
    ...
    while True:
        long_pair: tuple[int, int] | None = None
        for left_index in range(len(boundaries) - 1):
            right_index = left_index + 1
            if boundaries[right_index].time - boundaries[left_index].time > config.max_seconds:
                long_pair = (left_index, right_index)
                break
        if long_pair is None:
            return

        left_index, right_index = long_pair
        left = boundaries[left_index]
        right = boundaries[right_index]
