...
def _build_timeline(
    ...
    for kind, rows in (("speech", speech), ("nonverbal", nonverbal)):
        for index, row in enumerate(rows):
            ...
            if not text:
                continue
            start = _finite_time(row.get("start"), fallback=float(index))
...
def _validate_result(payload: Mapping[str, Any], target_ids: Sequence[str]) -> list[dict[str, str]]:
    ...
    for index, raw in enumerate(raw_segments):
        ...
        result.append(
            {
                "id": row_id,
                "corrected_text": _normalize_text(corrected),
                "status": status,
            }
        )
    if actual != expected:
...
def _safe_change(before: str, after: str, config: CorrectionConfig) -> tuple[bool, float, float]:
    compact_before = _compact(before)
    ...
    *,
    ...
    for row_id, item in by_id.items():
        ...
        if suggestion is None:
            ...
            requested = _normalize_text(suggestion["corrected_text"])
