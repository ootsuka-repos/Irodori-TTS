...
class NonverbalLabelingConfig:
    ...
    seed_prob: float = 0.75
    seed_margin: float = 0.20
    k: int = 7
    min_cosine: float = 0.72
    neighbor_agreement: float = 0.65
    min_neighbors: int = 1
...
def propagate_nonverbal_labels(
    ...
    *,
    ...
    1. a manual label always wins; manual ``aegi/chupa`` labels also become seeds;
    ...
    for index, (segment_id, label, decision, prediction) in enumerate(
        ...
        evidence["direct_seed"] = {
            "seed_id": segment_id if label in TARGET_LABELS else None,
            "label": label,
            "source": "manual" if decision == "manual_override" else "classifier",
            "propagates": label in TARGET_LABELS,
        }
