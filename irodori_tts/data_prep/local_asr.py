...
class AnimeWhisperTranscriber:
    ...
    def transcribe(self, audio_paths: Sequence[Path]) -> list[str]:
        ...
        for output in values:
            ...
            result.append(_normalize_transcript(str(output.get("text", ""))))
        return result
...
def transcript_review_reasons(text: str) -> list[str]:
    """Flag empty or strongly repetitive local-ASR output without discarding it."""
    ...
    *,
    ...
    def process(entries: list[tuple[int, str, Path, dict[str, Any], Path]]) -> None:
        ...
        for entry, text in zip(entries, texts, strict=True):
            index, row_id, _audio_path, audio_metadata, path = entry
            normalized = _normalize_transcript(text)
            reasons = transcript_review_reasons(normalized)
            result = {
                "text": normalized,
            ...
            completed += 1
        print(
            f"anime-whisper rows={completed}/{len(pending)} cached={len(rows) - len(pending)}",
            flush=True,
        )
...
def config_metadata(config: AnimeWhisperConfig) -> dict[str, Any]:
    """Serializable public config used by the full-pipeline run record."""
