...
from irodori_tts.data_prep.acoustic_segmentation import ACOUSTIC_SEGMENTATION_VERSION
from irodori_tts.data_prep.ero_voice_classifier import SPACE_REPO_ID, SPACE_REVISION
...
class NonverbalTrainingManifestConfig:
    ...
    neighbor_minimum_count: int = 1
    neighbor_minimum_support: float = 0.65
    neighbor_minimum_similarity: float = 0.72
...
def _confidence_reasons(
    ...
    if isinstance(raw_statuses, Mapping) and raw_statuses:
        ...
        if not statuses or not statuses.issubset(TRUSTED_PRIMITIVE_STATUSES):
            return ["merged_provenance_untrusted"]
        # The finalizer retains accepted primitive status counts even when it
        # drops bulky per-primitive evidence after a safe same-label merge.
        return []
    ...
    if status == "neighbor_supported":
        return _neighbor_evidence_reasons(row, final_label, config)
    if status == "merged_same_label":
        return ["merged_provenance_missing"]
    return ["untrusted_label_status"]
...
```powershell
uv run --no-sync python -m irodori_tts.cli.merge_latent_manifests `
