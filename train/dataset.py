...
class LatentTextDataset(Dataset):
    ...
    def __init__(
        ...
        # Style-matched reference pool: same speaker AND same category. The
        # load-time filter above guarantees every kept sample's pool has >=2
        # clips, so no speaker-wide fallback pool is kept.
    ...
    def __getitem__(self, index: int) -> dict[str, Any]:
        ...
        if self.enable_speaker_condition:
            ...
            speaker_id = self.manifest_index.speaker_ids[sample_index]
            category = self.manifest_index.categories[sample_index]
            # Style-matched references only (same speaker + same category).
            # Load-time filtering guarantees every kept sample has >=2 clips in
            # its pool, so an undersized pool means the filter and this lookup
            # disagree — fail loudly instead of silently degrading to a
            # speaker-wide (style-mismatched) reference.
            candidates = self.speaker_category_to_indices.get((speaker_id, category), [])
            if len(candidates) <= 1:
                raise RuntimeError(
                    "style reference pool underflow for "
                    f"(speaker={speaker_id!r}, category={category!r}): "
                    f"{len(candidates)} clip(s); load-time filtering should "
                    "have removed this sample"
                )
            if len(candidates) > 1:
