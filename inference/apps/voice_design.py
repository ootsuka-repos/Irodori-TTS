...
from core.speaker_inversion import is_speaker_inversion_safetensors_path
from inference.apps.gradio_emoji_palette import EMOJI_PALETTE_CSS, build_emoji_palette
...
def _describe_runtime(
    ...
    if not runtime.model_cfg.use_caption_condition:
        notes.append(
            "warning: this checkpoint does not enable caption conditioning. Use irodori-web for reference-audio inference."
        )
...
def _run_generation(
    ...
    if not runtime.model_cfg.use_caption_condition:
        raise ValueError(
            "Loaded checkpoint does not enable caption conditioning. Use irodori-web for the original reference-audio model."
        )
