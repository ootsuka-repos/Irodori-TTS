...
from core.config import ModelConfig, merge_dataclass_overrides
...
from inference.runtime import _load_checkpoint_for_inference
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Export the EMA weights ('ema_model') stored by training with ema_enabled=true "
...
def main() -> None:
    ...
    if is_lora_adapter_dir(input_path):
        if bool(args.use_ema):
            raise ValueError("--use-ema is not supported for LoRA adapter checkpoints.")
        ...
        if bool(args.use_ema):
            ...
            if not isinstance(ema_state, dict):
                raise ValueError(
                    "--use-ema requested but the checkpoint has no 'ema_model' entry "
                    "(train with ema_enabled=true)."
                )
            payload = dict(payload)
            payload["model"] = ema_state
    ...
    if bool(args.use_ema):
        print("Exported EMA weights (ema_model) instead of raw training weights.")
