...
The examples below use the installed `irodori-*` commands. Optional direct Python
launchers are grouped under [`scripts/`](scripts/README.md).

### Simple Inference

```bash
uv run --no-sync irodori-infer \
...
```bash
uv run --no-sync irodori-web --server-name 0.0.0.0 --server-port 7860
```
...
```bash
uv run --no-sync irodori-voice-design-web --server-name 0.0.0.0 --server-port 7861
```
...
`irodori-web` is for `Aratako/Irodori-TTS-500M-v3`. `irodori-voice-design-web`
is for `Aratako/Irodori-TTS-600M-v3-VoiceDesign` and remains compatible with
v2 VoiceDesign checkpoints.
...
```bash
uv run --no-sync torchrun --nproc_per_node 4 -m irodori_tts.cli.train \
  --config configs/train_500m_v3_phase1_body.yaml \
  --manifest data/train_manifest.jsonl \
  --output-dir outputs/irodori_tts \
  --device cuda
...
Training supports YAML config files with `model` and `train` sections. CLI arguments take precedence over YAML values. See `uv run --no-sync irodori-train --help` for all available options.
...
```bash
uv run --no-sync irodori-convert-checkpoint outputs/checkpoint_final.pt
```

LoRA adapter checkpoints can also be converted directly:

```bash
uv run --no-sync irodori-convert-checkpoint outputs/irodori_tts_lora/checkpoint_final
```

LoRA adapter checkpoints are merged into the base model automatically during conversion, so the exported `.safetensors` file is directly usable for inference. If you do not want to merge the adapter, pass the adapter directory directly to `irodori-infer --lora-adapter` or the matching Gradio field.

## Project Structure

```text
Irodori-TTS/
├── irodori_tts/                 # Product code
│   ├── cli/                      # Train, infer, preprocess, and conversion CLIs
│   ├── apps/                     # Gradio applications
│   ├── inference/                # Runtime orchestration and post-processing
│   ├── training/                 # Device data path and training losses
│   ├── model.py, rf.py           # Architecture and Rectified Flow sampling
│   ├── dataset.py, tokenizer.py  # Manifest data path and text tokenization
│   ├── latents.py, codec.py      # Tensor layouts and DACVAE integration
│   └── config.py                 # Model / Train / Sampling configuration
├── configs/                     # Versioned training recipes
├── scripts/                     # Optional direct Python launchers
├── tests/                       # Fast-path regression tests
├── benchmarks/                  # Reproducible hot-path micro-benchmarks
├── docs/                        # Detailed guides
├── requirements/                # Legacy pip compatibility only
├── pyproject.toml               # Package, dependencies, and CLI entry points
└── uv.lock                      # Reproducible environment lock
```

Run the tail-detection benchmark with
`uv run --no-sync python -m benchmarks.benchmark_postprocessing`.
